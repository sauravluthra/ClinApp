...
def main():
    ...
    params = {
        'CSV_DIR' : 'imudata/41XG62/SubjectS/',
        'CSV_FILENAME' : "", #File Name of csv
        'IMU_COUNT' : 5, #Number of IMU sensors
        'FIRST_ROW': 6, #First row of data (Do NOT change)
        'LAST_ROW': 2005, #Last row of data to read
        'SAMPLING_TIME' : 0.005 #sampling time in seconds
    }

    #Iterate through each file in CSV_DIR and run it through fetchData() and runVQF
    for filename in os.listdir(params['CSV_DIR']):

        #Get absolute path for CSV file
        f = os.path.join(params['CSV_DIR'], filename)

        # checking if it is a file and if it is a .csv
        if os.path.isfile(f) and (".csv" in str(f)):
            params['CSV_FILENAME'] = str(f)
            print(f"ATTEMPTING TO READ FROM {params['CSV_FILENAME']}")
...
def runvqf(data, imuID, params):
    ...
    plt.plot(vqf['quat9D'])
    plt.grid()
    #plt.title(f'QUATERNION 9D w/BIAS ESTIMATE | IMU# {str(imuID)} | {str(params['CSV_FILENAME'])}')
    plt.tight_layout()
    plt.show()
    ...
    with open(f"output/QuatOutput9D_IMU{imuID}_{nowStr}_{params['CSV_FILENAME']}.csv", "w+"):
        np.savetxt(f"output/QuatOutput9D_IMU{imuID}_{nowStr}_{params['CSV_FILENAME']}.csv", vqf['quat9D'], delimiter = ",")
